 ...
 * 加入了luhm验证银行卡号
 * @author amterasu
 * @param  bankId String／Number 银行卡号
 * @return 对象 {code :num, msg: str}  code: 0:银行卡号为空 1:银行卡号格式不合法 2:未查询到所属银行 3.所属银行的名称
 */

class BankUtil {
  constructor() {
    this._bankBin = this._createBankBin();
    this._bankName = this._createBankName();
  }
  //传入卡号 得到银行名称
  _getNameOfBank(bankId) {
    let index = -1;
    let idCard = bankId + '';
    if (!idCard) {
      return {
    ...
    } else if (!this._luhmCheck(idCard)) {
      return {
        code: 1,
        msg: "银行卡号格式不合法"
      }
    ...
    //6位Bin号
    let cardbin_6 = idCard.substring(0, 6);
    for (let i = 0; i < this._bankBin.length; i++) {
      if (cardbin_6 == this._bankBin[i]) {
        index = i;
      }
    }
    if (index != -1) {
      return {
        code: 3,
        msg: this._bankName[index]
      }
    }
    //8位Bin号
    let cardbin_8 = idCard.substring(0, 8);
    for (let i = 0; i < this._bankBin.length; i++) {
      if (cardbin_8 == this._bankBin[i]) {
        index = i;
      }
    }
    if (index != -1) {
  ...
  getBankName(bankId) {
    return this._getNameOfBank(bankId)
  }
  //bankId为银行卡号 
  _luhmCheck(bankId) {
    let lastNum = bankId.substr(bankId.length - 1, 1);//取出最后一位（与luhm进行比较）
    let first15Num = bankId.substr(0, bankId.length - 1);//前15或18位
